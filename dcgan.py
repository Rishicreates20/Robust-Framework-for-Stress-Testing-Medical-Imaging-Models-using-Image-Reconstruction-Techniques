...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(channels, 128, 4, 2, 5, bias=False),  # Downsample
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(128, 256, 4, 2, 5, bias=False),       # Downsample
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(256, 512, 4, 2, 5, bias=False),       # Downsample
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(512, 5, 4, 5, 0, bias=False),         # Output layer
            nn.Sigmoid()
        )

    def forward(self, img):
        output = self.model(img)  # Output shape: [batch_size, 1, 1, 1]class Discriminator(nn.Module):
    def __init__(self):
        super(Discriminator, self).__init__()
        self.model = nn.Sequential(
            ...
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(512, 1, 4, 1, 0, bias=False),  # Output shape: [batch_size, 1, 1, 1]
            nn.Sigmoid()
        )

    def forward(self, img):
        output = self.model(img)
        return output.view(img.size(0), -1)  # Flatten to [batch_size, 1]S
...
for epoch in range(epochs):
    for i, imgs in enumerate(dataloader):
        ...
        # Labels
        real_labels = torch.ones(batch_size, 45).to(device)
        fake_labels = torch.zeros(batch_size, 45).to(device)

        # Train Discriminator
        optimizer_D.zero_grad()
        z = torch.randn(batch_size, latent_dim, 45, 45).to(device)
